...
# 一、项目背景与意义
在当前的计算机视觉领域，3D 理解是一个重要的研究方向。随着深度学习的发展，基于扩散模型生成的合成图像带有伪 3D 注释，这为 3D 研究提供了新的数据来源。然而，大概有四分之一的合成图像存在视点或形状不一致的问题，所以开发具有准确 3D 注释的数据集变得至关重要。<br>
本项目旨在创建这样一个数据集，用于评估基于现有大语言模型（LLMs）和生成扩散模型知识构建的 3D 理解模型（如物体定位、检测、分割等）的零样本 / 少样本性能。通过精确标注物体在 3D 空间中的位置、旋转以及其他属性，能够为相关研究提供高质量的数据支持，推动 3D 视觉技术在自动驾驶、虚拟现实、机器人感知等领域的应用发展。<br>
