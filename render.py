...
def proj2d(x3d, pose, viewport=2000.0):
    azim = pose['azimuth'] / 180.0 * np.pi
    elev = pose['elevation'] / 180.0 * np.pi
    theta = pose['theta'] / 180.0 * np.pi
    dist = pose['distance']
...
def matplotlib_line_render(img_path, mesh_path, pose, color=(0, 0, 100), alpha=0.4, add_margin=False, viewport=2000.0):
    # 144.77 / 1000
    verts, faces = load_off_np(mesh_path)
...
if __name__ == '__main__':
    pose = {'azimuth': 65.0, 'elevation': 5.0, 'theta': 0.0, 'distance': 16.0, 'px': 385.0, 'py': 330.0}
